    ...
    uint8_t dest_len, const void *payload, uint8_t length, uint8_t mcast)
{
    ...
    fcs = (uint16_t *)&buf[frame->payload_len + hdrlen + 1];
    *fcs = ieee802154_frame_get_fcs(buf, frame->payload_len + hdrlen + 1);
                            ...
                            uint8_t payload_len, uint8_t mcast)
{
    if (mcast) {
        ...
        if (dest_len == 8) {
            ...
            return net_if_send_packet(if_id, *((uint16_t *)dest),
                                      payload, (size_t)payload_len);
        }
    }
                                        ...
                                        uint8_t payload_len, uint8_t mcast)
{
    if (net_if_get_interface(if_id) &&
        ...
        uint8_t length;
        uint8_t hdrlen = sixlowpan_mac_prepare_ieee802144_frame(&frame, if_id,
