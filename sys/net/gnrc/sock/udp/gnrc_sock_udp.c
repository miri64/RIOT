...
static inline bool _af_not_supported(const sock_udp_ep_t *ep)
{
                    ...
                    const sock_udp_ep_t *remote, uint16_t flags)
{
    ...
    memset(&sock->local, 0, sizeof(sock_udp_t));
    if (local != NULL) {
#ifdef MODULE_GNRC_SOCK_CHECK_REUSE
        if (!(flags & SOCK_FLAGS_REUSE_EP)) {
            for (gnrc_udp_t *ptr = _udp_socks; ptr != NULL; ptr = ptr->next) {
                if (memcmp(ptr->local, local, sizeof(sock_udp_ep_t)) == 0) {
                    return -EADDRINUSE;
                }
            }
        }
        LL_PREPEND2(socks, sock, reg.next);
#endif
        if (_af_not_supported(local)) {
            return -EAFNOSUPPORT;
        }
        memcpy(&sock->local, local, sizeof(sock_udp_t));
        gnrc_sock_create(&sock->reg, GNRC_NETTYPE_UDP,
                         local->port, flags);
    }
    memset(&sock->remote, 0, sizeof(sock_udp_t));
    if (remote != NULL) {
        if (_af_not_supported(remote)) {
            return -EAFNOSUPPORT;
        }
        memcpy(&sock->remote, remote, sizeof(sock_udp_t));
    }
    sock->flags = flags;
    return 0;
}
                  ...
                  uint32_t timeout, sock_udp_ep_t *remote)
{
    ...
    if (((sock->remote.port != 0) &&    /* check remote end-point if set */
                 ...
                 const sock_udp_ep_t *remote)
{
    ...
    if ((remote != NULL) && (sock != NULL) &&
        (sock->local.netif != CONN_EP_ANY_NETIF) &&
        (remote->netif != CONN_EP_ANY_NETIF) &&
    ...
    if ((remote == NULL) && ((sock == NULL) || (sock->remote.port == 0))) {
        return -ENOTCONN;
    }
    if ((sock == NULL) || (sock->local.port == 0)) {
        while (src_port == 0) {
            ...
            if ((sock == NULL) || !(sock->flags & SOCK_FLAGS_REUSE_EP)) {
                ...
                for (gnrc_udp_t *ptr = _udp_socks; ptr != NULL; ptr = ptr->next) {
                    ...
                    /* int shouldn't overflow as it is either 16-bit or 32-bit
                     * => aligns with both IPv4 and IPv6, but we get some
                     *    compiler optimization */
                    for (int *c = (int *)ptr->local.addr; i < sizeof(ptr->local.addr); i++) {
                        if (c != 0) {
                            spec_addr = true;
                        }
                    }
