 ...
 * @detail      The idea of this module is that every network layer provides
 *              a basic set of commands to communicate with it's lower and
...
/**
 * @brief Definition of OK result for NETAPI_CMD_ACK
 */
 ...
 * @detail  This list may be expanded by a some layer (using values
...
typedef enum {
     ...
     *          receiving.
     *
     * @detail  If there is no space left in the control threads registry,
     ...
     *          to this layer.
     *
     * @detail  Control thread does not need to notify, if the thread in
     ...
     *
     * @detail  Use this command with the netapi_rcv_pkt_t command type.
     */
    NETAPI_CMD_FIRE_RCV,
#endif
} netapi_cmd_type_t;
 ...
 * @detail  Writers of new network protocols are adviced to choose new more
 ...
 *          result NETAPI_STATUS_OK if the operation was successfull or
...
typedef struct {
     ...
     * @detail  Valid values are NETAPI_STATUS_OK or a fitting negative
 ...
 * @extends netapi_cmd_t
 *
 * @detail  The netapi_snd_pkt_t::ulh will be prepended
...
typedef struct {
     ...
     * @detail In case of NETAPI_CMD_GET the data will be written here
...
typedef struct {
    /* cppcheck-suppress unusedStructMember because interface is not used yet */
    netapi_cmd_type_t type; /**< Type of the command. Must be
                                 NETAPI_CMD_REG or NETAPI_CMD_UNREG */
    netapi_ack_t *ack;      /**< Pointer to where the acknowledgement to this
                                 message is stored. Must not be NULL */
 ...
 *
 * @detail  The upper layer information will be prepended
 ...
 *          acknowledgement at all.
 */
int netapi_send_data2(kernel_pid_t pid, netdev_hlist_t *upper_layer_hdrs,
 ...
 *          acknowledgement at all.
 */
static inline int netapi_send_data(kernel_pid_t pid, void *addr,
                                   size_t addr_len, void *data,
                                   size_t data_len)
{
    return netapi_send_data2(pid, NULL, addr, addr_len, data, data_len);
}
 ...
 *          acknowledgement at all.
 */
int netapi_register_current_thread(kernel_pid_t pid);
 ...
 *          acknowledgement at all.
 */
int netapi_unregister_current_thread(kernel_pid_t pid);
