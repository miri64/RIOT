...
uint8_t lowpan_iphc_encoding(int if_id, const void *dest, int dest_len,
                            ...
                            uint8_t *data, uint16_t data_len)
{
    ...
    DEBUG("sixlowpan_lowpan_sendto(%d, %s, data, %"PRIu16"): send_packet_length: %"PRIu16", header_size: %"PRIu16"\n",
          if_id, ipv6_addr_to_str(addr_str, IPV6_MAX_ADDR_STR_LEN, dest),
          data_len, send_packet_length, header_size);
                             ...
                             ipv6_hdr_t *ipv6_buf_extra, uint8_t *ptr)
{
    ...
    uint16_t hdr_pos = 0;
    uint8_t tc;
    net_if_eui64_t own_addr_long;

    if (!net_if_get_eui64(&own_addr_long, if_id, 0)) {
        return 1;
    }
    ...
    if (ipv6_addr_is_unspecified(&(ipv6_buf->srcaddr))) {
        ...
        if (memcmp(&(ipv6_buf->srcaddr.uint8[8]), &own_addr_long, 8) == 0) {
    ...
    if (ipv6_addr_is_multicast(&ipv6_buf->destaddr)) {
        ...
        if ((con = lowpan_context_lookup(&ipv6_buf->destaddr)) != NULL) {
            ...
            net_if_eui64_t dest_iid;

            /* TODO: NOPE 16-bit short are not supported here  */
            if (dest_len == 8 &&
                (&ipv6_buf->destaddr.uint8[8] & 0xfd) == (dest[0] & 0xfd) &&
                memcmp(&ipv6_buf->destaddr.uint8[9], &dest[1], 8) == 0) {
                /* 0 bits. The address is derived using context information
                 * and possibly the link-layer addresses.*/
                lowpan_iphc[1] |= 0x03;
            }
