...
static void _set_up(void)
{
    ...
    _last_sent_frame = xtimer_now_usec() - GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US;
...
static void test_sixlo_recv_fwd__1st_frag_uncomp__req_ack(void)
{
     ...
     * - https://tools.ietf.org/html/draft-ietf-6lo-fragment-recovery-07#section-6.3
     */
...
static void test_sixlo_recv_fwd__ack__no_vrbe(void)
{
    ...
    TEST_ASSERT(0 < gnrc_netapi_dispatch_receive(GNRC_NETTYPE_SIXLOWPAN,
                                                 ...
                                                 ack));
    /*
     * https://tools.ietf.org/html/draft-ietf-6lo-fragment-recovery-07#section-6.2
...
static void test_sixlo_send(void)
{
    ...
    TEST_ASSERT(3 <= GNRC_SIXLOWPAN_SFR_OPT_WIN_SIZE);
    ...
    hdr = (sixlowpan_sfr_rfrag_t *)&_target_buf[mhr_len];
    TEST_ASSERT_EQUAL_INT(tag, hdr->base.tag);
    for (unsigned i = 0; i < GNRC_SIXLOWPAN_SFR_FRAG_RETRIES; i++) {
     ...
     * GNRC_SIXLOWPAN_SFR_FRAG_RETRIES retries */
...
static size_t _wait_for_packet(size_t exp_size)
{
    ...
    if (mhr_len > 0) {
        if ((GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US > 0U) &&
            (now - _last_sent_frame) < GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US) {
            puts("(now - _last_sent_frame) < GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US");
            return 0;
        }
