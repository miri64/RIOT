...
void ipv6_send_packet(ipv6_hdr_t *packet)
{
    ...
    if (!ipv6_addr_is_multicast(&ipv6_buf->destaddr) ||
        ipv6_addr_is_on_link(&ipv6_buf->destaddr, ipv6_buf->srcaddr)) {
        nce = ndp_get_ll_address(&bytes->destaddr);
        ...
        if (ipv6_addr_is_multicast(&ipv6_buf->destaddr)) {
            ...
            return sixlowpan_lowpan_sendto(0, 0, (uint8_t *)ipv6_buf, packet_length);
        }

        ipv6_addr_t *dest = ip_get_next_hop(&ipv6_buf->destaddr);
...
void ipv6_process(void)
{
    ...
    while (1) {
        ...
        /* destination is foreign address */
            ...
            sixlowpan_lowpan_sendto(nce->if_id, &nce->lladdr, nce->lladdr_len,
    ...
    while ((if_id = net_if_iter_interfaces(if_id)) >= 0) {
        while (net_if_iter_addresses(if_id, (net_if_addr_t **) &addr_entry) != NULL) {
            if (addr_entry->addr_protocol & NET_IF_L3P_IPV6) {
                if (memcmp(&addr_entry->addr_data, addr, 16) == 0) {
                    hit->if_id = if_id;
                    hit->addr = addr_entry;
                    return hit;
                }
