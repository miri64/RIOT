...
void _net_if_ifconfig_add(int if_id, char *type);
void _net_if_ifconfig_add_ipv6(int if_id, char *addr);
void _net_if_ifconfig_set(int if_id, char *key);
...
void _net_if_ifconfig_create(void);
...
static inline void _str_to_eui64(net_if_eui64_t *eui64, char *eui64_str)
{
    if (isnumber(eui64_str)) {
...
void _net_if_ifconfig(char *args)
{
    char *arg1;
    strtok(args, " ");
    arg1 = strtok(NULL, " ");
    ...
    if (arg1 == NULL) {
        ...
        if (cmd == NULL) {
            ...
            _net_if_ifconfig_add(if_id, type);
            return;
        }
        else if (strcmp(cmd, "set") == 0) {
            char *key = strtok(NULL, " ");
            _net_if_ifconfig_set(if_id, key);
            return;
        }
    }
...
void _net_if_ifconfig_add_ipv6(int if_id, char *type)
{
    char *addr_str;
    char *addr_data_str;
    char *addr_data_len;
    ...
    if (strcmp(type, "multicast") != 0 && strcmp(type, "anycast") != 0) {
        addr_str = type;
        type = NULL;
    }
    else {
...
void _net_if_ifconfig_add(int if_id, char *type)
{
    ...
    if (strcmp(type, "ipv6") == 0) {
        _net_if_ifconfig_add_ipv6(if_id, addr);
    }
    else {
        add_usage();
    }
...
void _net_if_ifconfig_create(void)
{
    ...
    while ((transceiver_str = strtok(NULL, ","))) {
...
void _net_if_ifconfig_list(int if_id)
{
    ...
    if (channel < 0 || pan_id < 0) {
        printf("Error reading interface %d\n", if_id);
    }

    printf("Iface %3d   HWaddr: 0x%04x Channel: %d PAN ID: 0x%04x\n", if_id,
           hw_address, (uint16_t)channel, (uint16_t)pan_id);
    printf("            EUI-64: %s\n", eui64_str);
